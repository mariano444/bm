...
def index():
    ...
    if form.validate_on_submit():
        ...
        try:
            bot.login()  # Iniciar sesión en Facebook
        except Exception as e:
            flash(f'Error al iniciar sesión: {str(e)}', 'danger')
            return redirect(url_for('index'))

        # Función para procesar las publicaciones y enviar actualizaciones al front-end
        def procesar_publicaciones():
            ...
            for i in range(num_publications):
                ...
                try:
                    ...
                    yield f"data: Publicación {i + 1}/{num_publications} completada.\n\n"
                except Exception as e:
                    yield f"data: Error en la publicación {i + 1}: {str(e)}\n\n"
                    continue

                time.sleep(random.uniform(30, 60))  # Esperar entre publicaciones

            bot.close_browser()
            yield "data: Publicaciones realizadas con éxito!\n\n"

        # Usar Server-Sent Events (SSE) para actualizar el front-end en tiempo real
        return Response(stream_with_context(procesar_publicaciones()), content_type='text/event-stream')
